...
* **购买后**： 不用动它保持默认。
* **切换模式**： 达到延迟时间后进行模式切换。
* **31发购买延迟**： 因为一直原价探价购买31发比较费资金，所以这里购买延迟可以自行调整(单位是毫秒。1秒=1000毫秒)。
...
* **自动运行功能**： 开启此功能后，如果勾选了自动登录，在网络波动时软件会自动重启-登录-运行功能。如果正常关闭软件，该功能会自动取消勾选，防止下次进入软件自动运行造成不必要的麻烦。
...
5.  主界面方案1里面设置入手价格，就是你要屯仓要设定在什么价格进行购买，购买次数就是屯仓数量，你补低价房卡、配件、装备可以设置1 ，买一个就停止运行了。屯子弹的话购买次数设置为0。挡屎价格是防止识别出错误购买用的，我们一般设置为入手价格的一半。
